...
if __name__ == '__main__':
    ...
    print("one_repo_name_list", len(own_config_repo_name_list),len(set(own_config_repo_name_list)),len(set(own_config_repo_name_list)))
    ...
    dict_test_case_inf = util.load_pkl(save_test_methods_dir_simplify, "all_test_case")
    remain_repo_list = []
    for repo in list(own_config_repo_name_list):
        if os.path.exists(save_test_methods_dir_success_own_config + repo + ".pkl"):
            dict_test_case_success = util.load_pkl(save_test_methods_dir_success_own_config, repo)
        else:
    ...
    pool.map(get_test_case_success_inf_own_config, remain_repo_list)  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    #'''
    '''
