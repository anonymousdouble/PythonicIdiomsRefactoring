...
class Rewrite_var_star(ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        ...
        return node
class Rewrite_var_star_insert_comehere(ast.NodeTransformer):
    def __init__(self, node_list, new_node, call_node):
            self.old_node = node_list
            self.new_node = new_node
            self.call_node = call_node
            self.flag = 0
            self.come_here_stmt = "print('I come here',end='')"
            for node in ast.walk(ast.parse(self.come_here_stmt)):
                if isinstance(node, ast.stmt):
                    self.come_here_stmt = node
                    break
    def generic_visit(self, node):
        if hasattr(node, "body"):
            ...
            for ind, child in enumerate(node.body):
                if child.lineno == self.call_node.lineno:  # and ast.unparse(child)==ast.unparse(self.call_node):
                    ...
                    node.body.insert(ind, self.come_here_stmt)
        ...
        if hasattr(node, "orelse") and not self.flag:
            ...
            for ind, child in enumerate(node.orelse):
                if child.lineno == self.call_node.lineno:  # and ast.unparse(child)==ast.unparse(self.call_node):
                    ...
                    node.orelse.insert(ind, self.come_here_stmt)
...
class Rewrite_ass(ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        ...
        return node
class Rewrite_compre_insert(ast.NodeTransformer):
        ...
        if hasattr(node, "body"):
            ...
            for ind, child in enumerate(node.body):
                if child.lineno == self.old_node.lineno and ast.unparse(self.old_node) in ast.unparse(
                    ...
                    if self.come_here_stmt in node.body:
                        break
                    #
                    node.body.insert(ind, self.come_here_stmt)
        ...
        if hasattr(node, "orelse") and not self.flag:
            ...
            for ind, child in enumerate(node.orelse):
                try:
                    if child.lineno == self.old_node.lineno and ast.unparse(self.old_node) in ast.unparse(
                            child):  # and ast.unparse(child)==ast.unparse(self.call_node):
                        if self.come_here_stmt in node.orelse:
                            break
...
class Rewrite_multiple_ass(ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        ...
        return node
class Rewrite_multiple_ass_insert_comehere(ast.NodeTransformer):
    def __init__(self,old_node,new_node):
        self.old_node =old_node
        self.new_node =new_node
        self.come_here_stmt = "print('I come here',end='')"
    ...
    def remove_node(self, cur_node):
        ...
        if beg != -1:
            for rem_node in remove_list:
                cur_node.remove(rem_node)
            cur_node.insert(beg, self.come_here_stmt)
        return cur_node
    def generic_visit(self, node):
        if hasattr(node, "body"):
            # print("node: ",ast.unparse(node))
        ...
        if hasattr(node, "body"):
            ...
            for ind,child in enumerate(node.body):
                ...
                if child.lineno==self.old_node.lineno:
                    ...
                    if not self.flag:  # ast.unparse(a)!=ast.unparse(node.body[ind]) and not self.flag:
        ...
        if hasattr(node, "orelse") and not self.flag:
            ...
            for ind, child in enumerate(node.orelse):
                if child.lineno == self.old_node.lineno:
                    ...
                    if not self.flag:  # ast.unparse(a) != ast.unparse(node.orelse[ind]):
    ...
    def generic_visit(self, node):
        if hasattr(node, "body"):
            ...
            for ind,child in enumerate(node.body):
                ...
                node.body[ind] = self.generic_visit(node.body[ind])
        return node
def replace_file_content_for_compre_3_category_insert_comehere(repo_name,file_html,for_node,assign_stmt,new_code,remove_ass_flag=0):
    ...
    rew = Rewrite_compre_insert(for_node, assign_stmt, new_code, remove_ass_flag)
...
class Rewrite_for_else_insert_time(ast.NodeTransformer):
    ...
    def __init__(self,child, child_copy,ass_init, if_varnode, init_ass_remove_flag):
        ...
        self.assign_init_zejun_tar=set([])
        self.assign_init_zejun=set([])
        ...
        if hasattr(node, "body"):
            ...
            for ind,child in enumerate(node.body):
                ...
                # for ind, child in enumerate(node.body):
                    ...
                    if ast.unparse(child) == ast.unparse(self.ass_init):
                        ...
                        # print(">>>>come for node here: ", ast.unparse(child),self.vars,self.assign_init_zejun_tar,self.assign_init_zejun)
                        index_for = node.body.index(child)
                        node.body.insert(index_for+2, self.come_here_stmt)
        ...
        if hasattr(node,"orelse")  and not self.flag:
            ...
            for ind,child in enumerate(node.orelse):
                ...
                if ast.unparse(child) == ast.unparse(self.ass_init):
                    ...
                    # print(">>>>come for node here: ", ast.unparse(child), self.vars, self.assign_init_zejun_tar,
                    #       self.assign_init_zejun)
                    index_for = node.orelse.index(child)
                    node.body.insert(index_for + 2, self.come_here_stmt)
                    #
                    # node.orelse.insert(index_for+2, self.print_total_stmt)
                    # node.orelse.insert(index_for+2, self.total_time_stmt_ass_init)
                    # node.orelse.insert(index_for+2, self.end_time_stmt)
                    #
                    # # node.orelse.insert(index_for, child)
                    # node.orelse.insert(index_for, self.start_time_stmt)
...
def replace_content_multi_ass_insert_comhere(repo_name,file_html,ass_list,new_code):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, "python_star_2000repo/", repo_name, "/", rela_path])
    content = util.load_file_path(file_path)
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_multiple_ass_insert_comehere(ass_list, new_code)
    all_tree = rew.visit(all_tree)
    print("whether code1 same: ", old_tree_str == ast.unparse(all_tree))
...
def replace_content_chain_compar_insert_comehere(repo_name,file_html,compl_node,sim_node):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, "python_star_2000repo/", repo_name, "/", rela_path])
    content = util.load_file_path(file_path)
...
def replace_content_chain_compar(repo_name,file_html,compl_node,sim_node):
    ...
    #
    # return content, "\n".join(res_copy)
def replace_content_truth_value_test_insert_comehere(repo_name,file_html,compl_node,sim_node):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, "python_star_2000repo/", repo_name, "/", rela_path])
    content = util.load_file_path(file_path)
        ...
        if hasattr(node, "orelse") and not self.flag:
            ...
            for ind, child in enumerate(node.orelse):
                try:
                    ...
                    traceback.print_exc()
        return node
def replace_content_var_unpack_for_target_insert_comehere(repo_name,file_html,old_tree,new_tree):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, "python_star_2000repo/", repo_name, "/", rela_path])
    content = util.load_file_path(file_path)
...
def replace_content_var_unpack_call_star_insert_comehere(repo_name,file_html,arg_seq,sim_node,call_node):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, "python_star_2000repo/", repo_name, "/", rela_path])
    print("file_html: ",file_html)
    ...
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_var_star_insert_comehere(arg_seq, sim_node,call_node)
    all_tree = rew.visit(all_tree)
    print("whether code1 same: ", old_tree_str == ast.unparse(all_tree))
