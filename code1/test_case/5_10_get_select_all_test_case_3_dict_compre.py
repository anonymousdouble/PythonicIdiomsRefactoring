...
def get_test_case_inf():
    ...
    for repo_name in dict_test_case_inf:
        if repo_name not in remain_repo_name_list:
            continue
...
if __name__ == '__main__':
    ...
    for ind,file in enumerate(os.listdir(evaluation_testing_dir)[:]):
        if "list_compre" == file or "set_compreh" == file:
            print(file)
        ...
        # break
    print("one_repo_name_list", len(one_repo_name_list),len(set(one_repo_name_list)),len(set(one_repo_name_list)-set(repo_name_list)))
    ...
    # remain_repo_name_list=list(set(one_repo_name_list)- {file[:-4] for file in os.listdir(save_test_methods_dir_success)})
    ...
    remain_repo_name_list = set(dict_test_case_inf_selct.keys()) - set(
        [e[:-4] for e in os.listdir(save_test_methods_dir_success)])
    print("remain_list: ", remain_repo_name_list, len(remain_repo_name_list))
    # print("remain_repo_name_list: ", len(remain_repo_name_list))
    # remain_repo_name_list=set(one_repo_name_list)-set(repo_name_list)
    ...
    #'''
    dict_test_case_inf=util.load_pkl(save_test_methods_dir_simplify, "all_test_case")
    # for repo in dict_test_case_inf
    # print(len(dict_test_case_inf.keys()))
    #'''
    dict_success_run_test_case=dict()
    pro_path = util.data_root + "python_star_2000repo/"
    pool = newPool(nodes=30)
    pool.map(get_test_case_success_inf, list(remain_repo_name_list)[:])  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
