...
def save_code_map_test_cases(repo_name):
    ...
    slow_test_html_list = []
    for file_html,code_cl,code_me, (arg_list, new_tree) in dict_test_case_inf_selct[repo_name]:
        #'''
        arg_seq = arg_list[0]
        call_node = arg_list[-2]
        old_content, new_content, flag_same = \
            c_replace_content_by_ast_5_10.replace_content_var_unpack_call_star_insert_comehere(
                ...
                for cl, me in dict_success_run_test_case[test_html]:
                    ...
                    if flag_pass and through_flag:
                        ...
                        if code_me not in dict_code_map_pass_test_case[file_html][code_cl]:
                            ...
                            # (arg_list, new_tree)
                        if (tuple(arg_seq), new_tree) not in \
                                dict_code_map_pass_test_case[file_html][code_cl][code_me]:
                            dict_code_map_pass_test_case[file_html][code_cl][code_me][
                                (tuple(arg_seq), new_tree)] = []
                        dict_code_map_pass_test_case[file_html][code_cl][code_me][
                            (tuple(arg_seq), new_tree)].append([test_html, cl, me])
...
if __name__ == '__main__':
    ...
    dict_code_map_pass_test_case_code_dir = util.data_root + "5_10_all_test_cases/select/map_code_info_test_case/call_star/"
    ...
    for repo_name in list(set(selct_repo_list[:])&set(repo_exist_test_case)):
        ...
        for file_html, code_cl, code_me, code in dict_test_case_inf_selct[
            ...
            for code_node in dict_code_map_pass_test_case[file_html][code_cl][code_me]:
                ...
                # old_code = file_html + str(arg_str_list)
                if old_code == file_html + str(arg_str_list):
