...
def make_pytest_test(pro_path, relative_test_file_path, file_name,v, fun_list=[]):
    ...
    cmd_test += ["cd ", pro_path + relative_test_file_path,
                 " ;"]  # echo '", split_test_str, "' ; "]
    if not fun_list:
        cmd_test += ["nox -e 'pytest-zeromq-3(coverage=False)' -- ", file_name, " ;"]
        pass
    else:
        ...
        cmd_test += ["nox -e 'pytest-zeromq-3(coverage=False)' -- ", " ".join(fun_list), " ;"]
        # for fun in fun_list:
...
def rerun_pytest(std_out_res):
    ...
    return None
def configure_pro(pro_path,relative_test_file_path,ven_name,v = "7",export_python=True):
