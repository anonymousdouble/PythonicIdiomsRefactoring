...
if __name__ == '__main__':
    ...
    # if os.path.exists(save_test_file_res_dir)
    # os.mkdir(save_test_file_res_dir)
    pip_install_again = 1
    ...
    repo_list = []
    for file_name in sorted(os.listdir(save_me_test_me_dir)):
        repo_name = file_name[:-4]
        # if  repo_name!="jetson_stats":
        #     continue

        if os.path.exists(save_me_test_me_dir + repo_name + ".pkl") and repo_name=="AugLy":
            repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir,"venv_test_require_new_chain_ass_", False,
                              pip_install_again])
    print("number we need to configure projects: ", len(repo_list),[e for e,*o in repo_list])
    ...
    # test_html="https://github.com/smicallef/spiderfoot/tree/master/test/unit/spiderfoot//test_spiderfootthreadpool.py"
    # pro_path=util.data_root+"python_star_2000repo/spiderfoot/"
    # configure_pro_envir_util.run_test_file(test_html,pro_path)
