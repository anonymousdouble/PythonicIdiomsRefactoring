...
import time
import util, github_util,get_test_case_acc_util,configure_pro_envir_util,count_test_case_each_pro
                        ...
                        if total_name in dict_complica_me_list[file_html]:
                            ...
                            if complicate_code[file_html][cl][me] and test_me_inf_list:
                                ...
                                for ind, (old_tree, new_tree, break_list_in_for) in enumerate(
                                    ...
                                    old_content, new_content,flag_same = replace_content_by_ast.replace_content_for_else(repo_name, file_html, old_tree,new_tree)
                                    if flag_same:
        ...
        for file_html in complicate_code:
            ...
            if file_html in dict_complica_me_list:
                ...
                if one_repo_end_time - one_repo_start_time > 60 * 60:
                    ...
                    break
                # if file_html!="https://github.com/sympy/sympy/tree/master/sympy/tensor/array/expressions/conv_array_to_matrix.py":#"https://github.com/pymc-devs/pymc/tree/master/pymc/sampling.py":#"https://github.com/smicallef/spiderfoot/tree/master//sflib.py":#"https://github.com/smicallef/spiderfoot/tree/master//sfwebui.py":#
                #     continue
                        ...
                        if total_name in dict_complica_me_list[file_html]:
                            ...
                            if complicate_code[file_html][cl][me] and test_me_inf_list:
                                ...
                                for ind, (old_tree, new_tree, break_list_in_for, child, child_copy,
                                          ass_init, if_varnode, init_ass_remove_flag) in enumerate(
                                    complicate_code[file_html][cl][me]):
                                    complic_code_count += 1
                                    ...
                                    try:
                                        for test_html,each_rela_path, cl, me in test_me_inf_list:
                                            ...
                                            if not flag_pass:
                                                ...
                                                res.append(
                                                    [repo_name, file_html, cl, me, child.lineno,
                                                     ast.unparse(ass_init) + "\n" + ast.unparse(
                                                         child) + "\n" + ast.unparse(if_varnode),
                                                     ast.unparse(child_copy),
                                                     init_ass_remove_flag, 0, test_me_inf_list])
                                            ...
                                            res.append(
                                                [repo_name, file_html, cl, me, child.lineno,
                                                 ast.unparse(ass_init) + "\n" + ast.unparse(
                                                     child) + "\n" + ast.unparse(if_varnode), ast.unparse(child_copy),
                                                 init_ass_remove_flag, 1,
                                                 test_me_inf_list])  # print(repo_path + rela_path)
...
if __name__ == '__main__':
    ...
    save_me_test_me_dir = util.data_root + "methods_test_method_pair/for_else_improve/"
    ...
    save_test_file_res_dir=util.data_root + "save_test_file_res_dir/for_else_improve/"
    ...
    complicated_code_dir_pkl = util.data_root + "transform_complicate_to_simple_pkl_improve/for_else/"
    ...
    save_acc_res_csv_dir = util.data_root + "acc_res_compli_test_case/for_else_complicated_improve.csv"

    save_test_acc_dir= util.data_root + "acc_res_compli_test_case/for_else_complicated_acc_dir/"
    save_test_acc_dir= util.data_root + "acc_res_compli_test_case/for_else_complicated_acc_dir_improve/"
    save_test_acc_dir = util.data_root + "test_case_benchmark_dir/for_else_acc_dir/"

    again_repo_list = count_test_case_each_pro.get_fail_projects(save_me_test_me_dir, save_test_acc_dir)#util.data_root + "acc_res_compli_test_case/for_else_complicated_acc_dir_improve/"
    print("again_repo_list: ", again_repo_list)
    print("len: ", len(again_repo_list), len(os.listdir(save_me_test_me_dir)))
    # '''
    ...
    for file_name in os.listdir(save_me_test_me_dir):
        ...
        if repo_name not in again_repo_list:
            continue
        repo_list.append(file_name)
    print("again_repo_list: ", len(repo_list))
