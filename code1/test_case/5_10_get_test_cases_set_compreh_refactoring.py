...
if __name__ == '__main__':
    ...
    complicate_code_dir = util.data_root + "5_10_all_test_cases/code_info_3215/set_compreh/"
    dict_code_map_pass_test_case_code_dir=util.data_root + "5_10_all_test_cases/map_code_info_test_case/set_compreh/"
    result_3215_test_case_dir=util.data_root + "5_10_all_test_cases/refactor_result_test_case/set_compreh/"
    ...
    pool.map(run_test_case, repo_name_list)  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    ...
    acc_count=0
    total_count=0
    for file_name in os.listdir(result_3215_test_case_dir):
        repo_name=file_name[:-4]
        dict_code_map_pass_test_case = util.load_pkl(result_3215_test_case_dir, repo_name)
        for e in dict_code_map_pass_test_case:
            # print(e)
            acc_count += e[-2]
            total_count += 1
    print("refactoring result: ", total_count, acc_count)
    # a_5_10_count_acc.count_acc()
    '''
                            ...
                            for test_html, cl, me in dict_code_map_pass_test_case[file_html][code_cl][code_me][code_node]:
                                ...
                                if not flag_pass:
                                    res.append(
                                        [repo_name, file_html, code_cl, code_me, assign_node.lineno,
                                         ast.unparse(assign_node) + "\n" + ast.unparse(for_node),
                                         ast.unparse(new_tree), remove_ass_flag, 0, dict_code_map_pass_test_case[file_html][code_cl][code_me][code_node]])
                                    break
                            else:
                                res.append(
                                    [repo_name, file_html, code_cl, code_me, assign_node.lineno,
                                     ast.unparse(assign_node) + "\n" + ast.unparse(for_node),
                                     ast.unparse(new_tree), remove_ass_flag, 1,
                                     dict_code_map_pass_test_case[file_html][code_cl][code_me][code_node]])  # print(repo_path + rela_path)
                                # break
                            util.save_file_path(old_path, old_content)
                        except:
                            util.save_file_path(old_path, old_content)
        print(res)
        util.save_pkl(dict_code_map_pass_test_case_code_dir, repo_name, res)
    '''
