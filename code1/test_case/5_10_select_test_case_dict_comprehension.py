...
def save_code_map_test_cases(repo_name):
    ...
    slow_test_html_list = []
    for file_html,code_cl,code_me, (for_node, assign_node, remove_ass_flag, new_tree) in dict_test_case_inf_selct[repo_name]:
...
if __name__ == '__main__':
    ...
    dict_code_map_pass_test_case_code_dir = util.data_root + "5_10_all_test_cases/select/map_code_info_test_case/dict_compreh/"
    # util.save_pkl(dict_code_map_pass_test_case_code_dir, "test", dict())
    set_compre_repo=["buildbot",
    "mlxtend.pkl",
    "neat-python.pkl",
    "nilearn.pkl",
    "pipreqs.pkl",
    ...
    repo_list = set([])
    count = 0
    repo_name_list = list(dict_test_case_inf_selct.keys())#[file[:-4] for file in os.listdir(save_test_methods_dir_success)]
    print("len: ", len(dict_test_case_inf_selct.keys()),set(dict_test_case_inf_selct.keys())-set(repo_name_list))
    print("intersect with set comprehension: ",set(dict_test_case_inf_selct.keys()),set(dict_test_case_inf_selct.keys())&set(set_compre_repo))
    ...
    #             for node in dict_code_map_pass_test_case[file_html][code_cl][code_me]:
    #                 if ast.unparse(new_tree) == ast.unparse(node[-1]):
    #                     print("come node: ",str(node))
    #                 else:
    #                     print("node: ",node)
    #             break
    #             if dict_code_map_pass_test_case[file_html][code_cl][code_me][(for_node, assign_node, remove_ass_flag, new_tree)]:
    #                 print("len: ",len(dict_code_map_pass_test_case[file_html][code_cl][code_me][(for_node, assign_node, remove_ass_flag, new_tree)]))
    #             else:
    #                 print("dont cover")
    #
    #         except:
    ...
    print("len: ",len(selct_repo_list))
    #'''
    pool = newPool(nodes=30)
    pool.map(save_code_map_test_cases, selct_repo_list)  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    #'''
