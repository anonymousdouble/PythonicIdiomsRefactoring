...
def save_code_map_test_cases(repo_name):
    ...
    for file_html in complicate_code:
        # if file_html != "https://github.com/sympy/sympy/tree/master/sympy/polys/distributedmodules.py":#"https://github.com/pydicom/pydicom/tree/master/pydicom/fileset.py": #and file_html != "https://github.com/pydicom/pydicom/tree/master/pydicom/dataset.py":
        #     continue
        print(file_html)
            ...
            for code_me in complicate_code[file_html][code_cl]:
                for ind, (for_node, assign_node, remove_ass_flag, new_tree) in enumerate(
                    ...
                    old_content, new_content, flag_same = c_replace_content_by_ast_5_10.replace_file_content_for_compre_3_category_insert_comehere(
                        repo_name, file_html, for_node, assign_node, new_tree, remove_ass_flag)#replace_file_content_for_compre_3_category_insert_comehere
                    # print(file_html)
                    # '''
                    ...
                    try:
                        for test_html in dict_success_run_test_case:
                            # if test_html != "https://github.com/sympy/sympy/tree/master/sympy/polys/tests/test_distributedmodules.py":#'https://github.com/pydicom/pydicom/tree/master/pydicom/tests/test_dataset.py' and test_html != "https://github.com/pydicom/pydicom/tree/master/pydicom/tests/test_fileset.py":
                            #     continue
                            for cl, me in dict_success_run_test_case[test_html]:
                                # if me != "test_validate_and_correct_file_meta" and me != "test_bad_file_id":
                                #     continue
                                # if
                                # if "chain_criterio" not in me:
                                #     continue
...
if __name__ == '__main__':
    ...
    save_test_methods_dir_success_own_config = util.data_root + "5_10_all_test_cases/success_test_cases_own_config_false/"
    complicate_code_dir = util.data_root + "5_10_all_test_cases/code_info_3215/set_compreh_own_config/"
    dict_code_map_pass_test_case_code_dir=util.data_root + "5_10_all_test_cases/map_code_info_test_case/set_compreh_own_config_false/"
    set_compre_dir_list=["set_compreh","set_compreh_own_config"]
    repo_name_list = [file[:-4] for file in os.listdir(complicate_code_dir)]
    print("len: ",len(repo_name_list))
    ...
    pool.join()
    '''
    remain_list=[e for e in own_config_repo_name_list if e+".pkl" in os.listdir(dict_code_map_pass_test_case_code_dir)]
    # remain_list=[own_config_repo_name_list]
    get_code_map_test_case_info()
    count_test_case = 0
    old_test_case = 0
    count_code = 0
    for repo_name in remain_list[:7]:  # 4
        print(repo_name)
        ...
        for file_html, code_cl, code_me, code in dict_test_case_inf_selct[
            ...
            for code_node in dict_code_map_pass_test_case[file_html][code_cl][code_me]:
                print(code_node)
                if old_code == file_html + ast.unparse(code_node[0]):
                    count_test_case += len(dict_code_map_pass_test_case[file_html][code_cl][code_me][code_node])
                    break
    print("count: ", old_test_case, count_test_case, count_code)
