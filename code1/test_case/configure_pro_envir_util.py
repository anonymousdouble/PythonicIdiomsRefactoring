...
def create_virtual_envi(pro_path, v_name, v="9"):
    return "".join(
        ["cd ", pro_path, f" ; python3.", v, " -m venv ", v_name, v, " ; . ./", v_name, v, "/bin/activate;"])
    # return "".join(["cd ", pro_path, "&& python2 -m pip install virtualenv && python2 -m virtualenv venv_test && . ./venv_test/bin/activate"])
def activate_virtual_envi(pro_path, v_name, v="9"):
    return "".join(
        ["cd ", pro_path, " ; . ./", v_name, v, "/bin/activate;"])
    # return "".join(["cd ", pro_path, "&& python2 -m pip install virtualenv && python2 -m virtualenv venv_test && . ./venv_test/bin/activate"])
...
def pip_install(pro_path,v):
    ...
    # cmd_pip_pack = ["echo '", split_install, "' ;","sudo apt-get install python3.",v,"-dev -y;pip3 install pytest pytest-mock requests yarg pytest-cov; python3 /mnt/zejun/smp/code1/test_case/pipreqs.py; "]
        cmd_pip_pack = ["echo '", split_install, "' ;",f"pip3 install pytest pytest-mock requests yarg pytest-cov; rm {pro_path}requirements_my_gen.txt; python3 {util.code_root}test_case/pipreqs.py; "]
        cmd_pip_pack = ["echo '", split_install, "' ;",
                    f"pip3 install pytest pytest-mock requests yarg pytest-cov; "]

    else:
        cmd_pip_pack = ["echo '", split_install, "' ;",f"pip3 install pytest pytest-mock requests yarg pytest-cov;  python3 {util.code_root}test_case/pipreqs.py; "]
...
def make_pytest_test(pro_path, relative_test_file_path, file_name,v, fun_list=[]):
    ...
    if not fun_list:
        ...
        cmd_test += ["python3", " -m pytest -v ", file_name, " ;"]
        pass
    else:
        ...
        cmd_test += ["python3", " -m pytest -v ", " ".join(fun_list), " ;"]
    ...
    cmd_test = "".join(cmd_test + [" echo '", split_test_str, "';"])
    return cmd_test
def make_pytest_test_capture_output(pro_path, relative_test_file_path, file_name,v, fun_list=[],prefine_cpus=""):
    split_test_str = "*************************test*************************"
    cmd_test = ["echo '", split_test_str, "' ; "]
    whether_same = relative_test_file_path.split("/")
    if 1:  # len(set(whether_same))==len(whether_same):#可能有同名的module名https://github.com/nodejs/node-gyp/tree/master/gyp/pylib/gyp/input_test.py
    ...
    cmd_test += ["cd ", pro_path + relative_test_file_path,
                 " ;"]  # echo '", split_test_str, "' ; "]
    if not fun_list:
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest -s -v ", file_name, " ;"]
        pass
    else:

        # cmd_test="".join(["cd ", pro_path+relative_test_file_path," && ","python2 -m unittest ",file_name])
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest -s -v ", " ".join(fun_list), " ;"]
        # for fun in fun_list:
        #     cmd_test += ["python3.", v, " -m unittest -v ", file_name[:-3] + "." + fun, " ;"]
        # cmd_test = "".join(cmd_test + ["echo '", split_test_str, "'"])
    cmd_test = "".join(cmd_test + [" echo '", split_test_str, "';"])
    return cmd_test
def make_pytest_test_capture_output_each_fun(pro_path, relative_test_file_path, file_name,v, fun_list=[],prefine_cpus="",iterations=1):
    split_test_str = "*************************test*************************"
    cmd_test = ["echo '", split_test_str, "' ; "]
    whether_same = relative_test_file_path.split("/")
    if 1:  # len(set(whether_same))==len(whether_same):#可能有同名的module名https://github.com/nodejs/node-gyp/tree/master/gyp/pylib/gyp/input_test.py
        ...
        for ind, e in enumerate(fun_list):
            ...
            # print("fun_list[ind]: ",fun_list[ind])
        relative_test_file_path = ""
    str_iter=str(iterations)
    cmd_test += ["cd ", pro_path + relative_test_file_path,
                 " ;"," pip3 install pytest-flakefinder;"]  # echo '", split_test_str, "' ; "]
    if not fun_list:
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest  --flake-finder --flake-runs=",str_iter," -s -v ", file_name, " --cache-clear ;"]
        pass
    else:

        # cmd_test="".join(["cd ", pro_path+relative_test_file_path," && ","python2 -m unittest ",file_name])
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest --flake-finder --flake-runs=",str_iter," -s -v ", " ".join(fun_list), " --cache-clear ;"]
        # for fun in fun_list:
        #     cmd_test += ["python3.", v, " -m unittest -v ", file_name[:-3] + "." + fun, " ;"]
        # cmd_test = "".join(cmd_test + ["echo '", split_test_str, "'"])
    cmd_test = "".join(cmd_test + [" echo '", split_test_str, "';"])
    return cmd_test
def make_pytest_test_capture_output_performance_repeat(pro_path, relative_test_file_path, file_name,v, fun_list=[],prefine_cpus="",repeat=10):
    split_test_str = "*************************test*************************"
    cmd_test = ["echo '", split_test_str, "' ; "]
    whether_same = relative_test_file_path.split("/")
    if 1:  # len(set(whether_same))==len(whether_same):#可能有同名的module名https://github.com/nodejs/node-gyp/tree/master/gyp/pylib/gyp/input_test.py
    ...
    cmd_test += ["cd ", pro_path + relative_test_file_path,
                 " ;"]  # echo '", split_test_str, "' ; "]
    if not fun_list:
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest -s -vv ", file_name, "   --cache-clear ;"]
        pass
    else:

        # cmd_test="".join(["cd ", pro_path+relative_test_file_path," && ","python2 -m unittest ",file_name])
        cmd_test += [prefine_cpus, " python3.", v, " -m pytest -s -vv ", " ".join(fun_list), " --cache-clear ;"]
        # for fun in fun_list:
        #     cmd_test += ["python3.", v, " -m unittest -v ", file_name[:-3] + "." + fun, " ;"]
        # cmd_test = "".join(cmd_test + ["echo '", split_test_str, "'"])
    cmd_test = "".join(cmd_test + [" echo '", split_test_str, "';"])
...
def rerun_pytest(std_out_res):
    ...
    return None
def configure_pro(pro_path,relative_test_file_path,ven_name,v = "7",export_python=False,pip_install_again=0):

    ven_dir="".join([pro_path, ven_name, v])
    if not os.path.exists(ven_dir) or pip_install_again:
...
def run_test_file(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=True,pip_install_again=0):
    output = ""
    # "cloud-custodian/"#"django/"#node-gyp#bert#sentence-transformers
    relative_test_file = "/".join(test_html.replace("//", "/").split("/")[6:])
    print("relative_test_file: ",relative_test_file)
    ...
    # '''
    cmd_virtu,cmd_pip_pack,cmd_export_python=configure_pro(pro_path, relative_test_file_path, ven_name,v,export_python=export_python,pip_install_again=pip_install_again)
...
def run_test_file_capture_output(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=True,prefine_cpus=""):
    output = ""
    # "cloud-custodian/"#"django/"#node-gyp#bert#sentence-transformers
    relative_test_file = "/".join(test_html.replace("//", "/").split("/")[6:])
    ...
    cmd_test = make_pytest_test_capture_output(pro_path, relative_test_file_path, file_name,v, fun_list,prefine_cpus=prefine_cpus)
    ...
    try:
        ...
        return "TimeoutExpired"
def run_test_file_capture_output_each_fun(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=True,prefine_cpus="",iterations=1,pip_install_again=0):
    output = ""
    # "cloud-custodian/"#"django/"#node-gyp#bert#sentence-transformers
    for each_fun in fun_list:
        relative_test_file = "/".join(test_html.replace("//", "/").split("/")[6:])
        ...
        # print("relative_test_file_path: ",relative_test_file_path)
        '''
        test_full_file_path = pro_path + relative_test_file

        content = util.load_file_path(test_full_file_path)
        print(f"***********content of test file {test_html} of {fun_list} functios: \n",content)
        '''
        file_name = relative_test_file.split("/")[-1]
        # '''
        cmd_virtu,cmd_pip_pack,cmd_export_python=configure_pro(pro_path, relative_test_file_path, ven_name,v,export_python=export_python,pip_install_again=pip_install_again)
        ...
        cmd_test = make_pytest_test_capture_output_each_fun(pro_path, relative_test_file_path, file_name,v, [each_fun],prefine_cpus=prefine_cpus,iterations=iterations)
        print(">>>>>>>>>>>>>>>cmd_test: ", cmd_test)
        # cmd_test +=["python3.7 - m scripts.run_backend_tests --test_target = scripts.linters.codeowner_linter_test.CodeownerLinterTests.test_missing_important_codeowner_path_from_list;"]
        total_cmd = "".join([cmd_virtu, cmd_export_python, cmd_pip_pack, cmd_test, "deactivate;"])
        try:
            ...
            result = subprocess.run(total_cmd, shell=True,timeout=5*60, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            std_out_res = result.stdout.decode("utf-8")
            # print("std_out_res: \n", std_out_res)
            std_error = result.stderr.decode("utf-8") if result.stderr else ""
            std_args = result.args
...
def run_test_file_capture_output_each_fun_Popen(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=True,prefine_cpus="",iterations=1):
    output = ""
    # "cloud-custodian/"#"django/"#node-gyp#bert#sentence-transformers
    for each_fun in fun_list:
        relative_test_file = "/".join(test_html.replace("//", "/").split("/")[6:])
        ...
        cmd_test = make_pytest_test_capture_output_each_fun(pro_path, relative_test_file_path, file_name,v, [each_fun],prefine_cpus=prefine_cpus)
        print(">>>>>>>>>>>>>>>cmd_test: ", cmd_test)
        # cmd_test +=["python3.7 - m scripts.run_backend_tests --test_target = scripts.linters.codeowner_linter_test.CodeownerLinterTests.test_missing_important_codeowner_path_from_list;"]
        total_cmd = "".join([cmd_virtu, cmd_export_python, cmd_pip_pack, cmd_test, "deactivate;"])
        try:
            ...
            timeout_value=10#15*60
            for _ in range(timeout_value):  # 30 seconds
            ...
            # std_args = result.args
            # output = "\n".join([std_out_res, std_error, std_args])
            std_out_res, std_error = result.communicate()
            std_out_res = std_out_res.decode("utf-8")
            std_error = std_error.decode("utf-8") if std_error else "None"
            print("result: ", std_out_res, std_error)
            ...
            return "TimeoutExpired"
    return output
def run_test_file_capture_output_performance(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=True,prefine_cpus=""):
    output = ""
    # "cloud-custodian/"#"django/"#node-gyp#bert#sentence-transformers
    relative_test_file = "/".join(test_html.replace("//", "/").split("/")[6:])
    ...
    cmd_test = make_pytest_test_capture_output_performance_repeat(pro_path, relative_test_file_path, file_name,v, fun_list,prefine_cpus=prefine_cpus)
    print(">>>>>>>>>>>>>>>cmd_test: ", cmd_test)
    # cmd_test +=["python3.7 - m scripts.run_backend_tests --test_target = scripts.linters.codeowner_linter_test.CodeownerLinterTests.test_missing_important_codeowner_path_from_list;"]
    total_cmd = "".join([cmd_virtu, cmd_export_python, cmd_pip_pack, cmd_test, "deactivate;"])
    try:
...
def get_time_list(run_test_result_new):
    ...
    return time_list
#对于一个test file, 获得所有通过测试用例的test_methods (PASSED)
def test_result_through_code(run_test_result):
    if "I come here" in run_test_result:
        return 1
    return 0
...
def save_repo_test_result(info_list):
    ...
    if os.path.exists(save_me_test_me_dir + repo_name + ".pkl"):# and not os.path.exists(save_test_file_res_dir+repo_name+ ".pkl"):
        ...
        for test_html in dict_all_test_html:
            ...
            try:
                run_test_result = run_test_file(test_html, pro_path, ven_name, v, list(fun_list),export_python,pip_install_again)
