...
def save_code_map_test_cases(repo_name):
    ...
    for i_repo,file_html in enumerate(complicate_code):
        # if i_repo>0:
        #     continue
...
if __name__ == '__main__':
    ...
    print("total success run test cases: ", repo_num, total_test_cases, success_run_test_cases)

    '''
    pool = newPool(nodes=30)
    pool.map(save_code_map_test_cases, list(repo_name_list)[:])  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    '''
