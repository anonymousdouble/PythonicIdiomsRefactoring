    ...
    def generic_visit(self, node):
        ...
        for ind_field, k in enumerate(node._fields):
            ...
            try:
                ...
                # print("here: ", k,v,ast.unparse(v) )
                if isinstance(v, ast.AST):
                    # if v._fields:
                        # if flag:
                        #     print("come v: ", ast.unparse(v))

                    setattr(node, k, self.generic_visit(v))
                    # node._fields[k] = self.generic_visit(v)
                    pass
                elif isinstance(v, list):
                    # print("here list: ", k, v, ast.unparse(v))
                    for ind, e in enumerate(v):
                        # if flag:
                        #     print("come e: ", ast.unparse(e))
                        # print("here e: ", k, v, ast.unparse(e))
...
def replace_content_var_unpack_for_target(repo_name,file_html,old_tree,new_tree):
    ...
    all_tree = ast.parse(content)
    old_tree_str=ast.unparse(all_tree)
    # old_tree_new_str=old_tree_str.replace("raise IllegalConfiguration('Cannot pass both select and ignore. They are mutually exclusive.')","raise IllegalConfiguration('Cannot pass both select and ignore. ' 'They are mutually exclusive.')")
    # print("old_tree_str:\n ",old_tree_str)
    rew = Rewrite(old_tree, new_tree)
    all_tree = rew.visit(all_tree)
    print("whether code1 same: ",old_tree_str==ast.unparse(all_tree))
...
#/mnt/zejun/smp/data/python_star_2000repo/nni/nni/algorithms/hpo/curvefitting_assessor/model_factory_copy_zejun.py
    for repo_name in dict_repo_file_python:
        if repo_name !="subliminal":#"pydocstyle":#"subliminal":#"udocker":#"sendgrid-python":#"cleanlab":#"chainerrl":#"subliminal":#"nibabel":#"mlens":#"pingouin":#"thunder":#"pingouin":#"mlens":#"patroni":#"cloud-custodian":# "sacred":#"docker-py":#"mitmproxy":#
            continue
