...
def save_code_map_test_cases(repo_name):
    ...
    slow_test_html_list = []
    for file_html,code_cl,code_me, (old_tree, new_tree, break_list_in_for,child, child_copy,
                                                  ass_init, if_varnode, init_ass_remove_flag) in dict_test_case_inf_selct[repo_name]:
        old_content, new_content, flag_same = c_replace_content_by_ast_5_10.replace_content_for_else_insert_time(repo_name, file_html,child, child_copy,
            ...
            for test_html in dict_success_run_test_case:
                ...
                for cl, me in dict_success_run_test_case[test_html]:
                    ...
                    if flag_pass and through_flag:
                        ...
                        if (child, child_copy) not in \
                                dict_code_map_pass_test_case[file_html][code_cl][code_me]:
                            dict_code_map_pass_test_case[file_html][code_cl][code_me][
                                (child, child_copy)] = []
                        dict_code_map_pass_test_case[file_html][code_cl][code_me][
                            (child, child_copy)].append([test_html, cl, me])
...
if __name__ == '__main__':
    ...
    dict_code_map_pass_test_case_code_dir = util.data_root + "5_10_all_test_cases/select/map_code_info_test_case/for_else/"
    ...
    for repo_name in remain_list[:]:
        ...
        for file_html, code_cl, code_me, code in dict_test_case_inf_selct[
            ...
            for code_node in dict_code_map_pass_test_case[file_html][code_cl][code_me]:
                print(code_node)
                if ast.unparse(code[4]) == ast.unparse(code_node[1]):
