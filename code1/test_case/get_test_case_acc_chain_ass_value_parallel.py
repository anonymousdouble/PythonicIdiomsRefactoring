...
首先获得有可以运行的测试用例的complicated code的测试用例信息
'''
def get_each_repo_test_acc(info_list):
    file=info_list[0]
                ...
                for cl in complicate_code[file_html]:
                    ...
                    for me in complicate_code[file_html][cl]:
                        ...
                        if total_name in dict_complica_me_list[file_html]:
                            ...
                            test_me_inf_list=dict_complica_me_list[file_html][total_name]
                            print(">>>>>test_me_inf_list: ",test_me_inf_list,"\n>>>>>>>",complicate_code[file_html][cl][me])
                            if complicate_code[file_html][cl][me] and test_me_inf_list:
                                ...
                                for ind, (ass_list, new_tree_content) in enumerate(
                                    ...
                                    for child in ast.walk(ast.parse(new_tree_content)):
                                        if isinstance(child, ast.Assign):
                                            ...
                                            break
                                    # print("come to complicate_code: ", file_html)
                                    # print(">>>>>>>>>>>>>>>>>>>>new_tree:\n ", new_tree.lineno, ast.unparse(new_tree))  # ast.unparse(old_tree) old_tree.lineno,
                                    print(">>>>>>>>>>>>>>>>>>>>old_tree:\n ",
                                          [ast.unparse(ass) for ass in ass_list],ast.unparse(new_tree))  # old_tree.lineno,arg_list
                                    '''
                                    ...
                                    try:
                                        for test_html,each_rela_path, cl, me in test_me_inf_list:
                                            ...
                                            if not flag_pass:
                                                ...
                                                print("*******************it is failed: ", ass_str, new_tree_content, run_test_result)
                                                ...
                                                res.append(
                                                    [repo_name, file_html, cl, me, ass_list[0].lineno,
                                                     "\n".join(ass_str), new_tree_content, 0, test_me_inf_list])
                                            ...
                                            res.append(
                                                [repo_name, file_html, cl, me, ass_list[0].lineno,
                                                 "\n".join(ass_str), new_tree_content, 1, test_me_inf_list])
...
if __name__ == '__main__':
    ...
    save_me_test_me_dir= util.data_root + "methods_test_method_pair/chain_ass_same_value_complicated/"

    save_test_file_res_dir=util.data_root + "save_test_file_res_dir/multip_assign_complicated/"
    save_test_file_res_dir=util.data_root + "save_test_file_res_dir/chain_ass_same_value_complicated/"

    complicated_code_dir_pkl=util.data_root + "transform_complicate_to_simple_pkl/multip_assign_complicated/"
    complicated_code_dir_pkl= util.data_root + "transform_complicate_to_simple_pkl/chain_ass_same_value/"

    save_acc_res_csv_dir = util.data_root + "acc_res_compli_test_case/multip_assign_complicated.csv"
    save_acc_res_csv_dir = util.data_root + "acc_res_compli_test_case/chain_ass_same_value.csv"
    ...
    save_test_acc_dir = util.data_root + "test_case_benchmark_dir/chain_ass_same_value_complicated_acc_dir/"
    ...
    if not os.path.exists(save_test_acc_dir):
        ...
        os.makedirs(save_test_acc_dir)
    # repo_name_list=['routersploit','featuretools','moto',
    #            'django-rest-framework'
    #            ,'networkx','apprise','mlxtend','pymc']
    for file in os.listdir(save_test_file_res_dir):
        # if file[:-4] not in repo_name_list:
        #     continue
        repo_list.append([file,save_test_file_res_dir,save_me_test_me_dir,complicated_code_dir_pkl])
    print(">>>>repo_list: ",len(repo_list))
    # repo_list=configure_pro_envir_util.prepare_data(save_test_acc_dir,save_test_file_res_dir,save_me_test_me_dir,)
# https://github.com/JinnLynn/genpac/tree/master/tests/util.py actually Pass
    #这个2个是real wronghttps://github.com/devbisme/skidl/tree/master/skidl/netlist_to_skidl.py 'tmpl = "{ltab}{name} = Part(\'{lib}\'
    #这里的1个实际上都通过了/mnt/zejun/smp/data/python_star_2000repo/Neuraxle/examples/sklearn/plot_boston_housing_regression_with_model_stacking.py
    #这里的2个实际上都通过了 https://github.com/JinnLynn/genpac/tree/master/tests/util.py/mnt/zejun/smp/data/python_star_2000repo/genpac/tests/util.py
    #1个都是都有错误 https://github.com/Erotemic/ubelt/tree/master/ubelt/util_hash.py  ['base = _rectify_base(base)', 'hashlen = _rectify_hashlen(hashlen)', 'hasher = _rectify_hasher(hasher)()'] base , hashlen , hasher  = _rectify_base(base), _rectify_hashlen(hashlen), _rectify_hasher(hasher)()
    #这里的4个都是错误的"subliminal.pkl""tvdb.py"
    #这里的1个都是错误的"https://github.com/EducationalTestingService/skll/tree/master/tests/utils.py"
    # 这里的2个都是错误的/mnt/zejun/smp/data/python_star_2000repo/cloud-custodian/c7n/schema.py
