...
import time
import util, github_util,configure_pro_envir_util,count_test_case_each_pro
...
def pip_install(pro_path,v):
    ...
    cmd_pip_pack = ["echo '", split_install, "' ;","sudo apt-get install python3.",v,"-dev;pip3 install pytest pytest-mock requests yarg pytest-cov; python3 /mnt/zejun/smp/code1/test_case/pipreqs.py; "]
    #'''
...
if __name__ == '__main__':
    ...
    save_test_acc_dir = util.data_root + "get_test_case_acc_for_compre_set_parallel.py/"
    #save_test_acc_dir = util.data_root + "acc_res_compli_test_case/var_unpack_call_star_complicated_acc_dir/"

    again_repo_list=count_test_case_each_pro.get_fail_projects(save_me_test_me_dir,save_test_acc_dir)
    ...
    for file_name in os.listdir(save_me_test_me_dir):
        ...
        if repo_name not in again_repo_list:
            continue
        # if os.path.exists(pro_path+repo_name+"/venv_zejun_config7"):
        #     continue# if repo_name!="sympy":
        #     continue
        repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir,"venv_zejun_config"])
        # if os.path.exists(save_me_test_me_dir + repo_name + ".pkl"):
        #     repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir])
    print("repo_list: ",len(repo_list))
    #'''
    pool = newPool(nodes=30)
    pool.map(configure_pro_envir_util.save_repo_test_result, repo_list)  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    time_end = time.time()
    print("configure project total time: ", time_end - time_start)
    #'''
    configure_pro_envir_util.get_test_success_count(save_me_test_me_dir, save_test_file_res_dir)
    time_end = time.time()
