...
if __name__ == '__main__':
    ...
    repo_list = []
    for file in os.listdir(save_test_file_res_dir):
        if file!="subliminal.pkl":
            continue
        # if os.path.exists(save_test_acc_dir+file):
        #     continue
        repo_list.append(file)
        get_each_repo_test_acc(file)
    print("we need to test: ",len(repo_list),len(list(set(repo_list))))
    # pool = newPool(nodes=30)
    # pool.map(get_each_repo_test_acc, list(set(repo_list))[:]) # [:3]sample_repo_url ,token_num_list[:1]
    # pool.close()
    # pool.join()
    #
