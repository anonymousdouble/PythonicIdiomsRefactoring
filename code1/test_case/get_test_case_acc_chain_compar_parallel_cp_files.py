                        ...
                        if total_name in dict_complica_me_list[file_html]:
                            ...
                            if complicate_code[file_html][cl][me] and test_me_inf_list:
                                ...
                                for ind, (old_tree, new_tree) in enumerate(
                                    ...
                                    if flag_same:
                                        try:
                                            print("file path of read content: ",repo_path+rela_path)
                                            content = util.load_file_path(repo_path+rela_path)

                                            real_rela_path = "/".join(real_file_html.split("/")[6:])
                                            file_path = "".join(
                                                [util.data_root, "python_star_2000repo/", repo_name, "/", real_rela_path])
                                            # shutil.copy(old_path,file_path)
                                            util.save_file_path(file_path, content)
