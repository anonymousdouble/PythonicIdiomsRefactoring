...
import util, github_util,configure_pro_envir_util,count_test_case_each_pro,configure_pro_envir_util_salt_own_config
...
def pip_install(pro_path,v):
    ...
    cmd_pip_pack = ["echo '", split_install, "' ;","pip3 install pytest pytest-mock requests yarg pytest-cov; python3 /mnt/zejun/smp/code1/test_case/pipreqs.py; "]
...
def run_test_file(test_html, pro_path,ven_name= "venv_test_require_",v='7',fun_list=[],export_python=False):
    output = ""
...
if __name__ == '__main__':
    ...
    for file_name in os.listdir(save_me_test_me_dir):
        ...
        if repo_name not in again_repo_list:
            continue
        if repo_name!="arcade":#"rosbridge_suite":#"CLUEPretrainedModels":#"clusterfuzz":#"salt":#"sympy":
            continue
        repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir,"venv_zejun_config",False])
        # if os.path.exists(save_me_test_me_dir + repo_name + ".pkl"):
        #     repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir])
    ...
    get_test_success_count(save_me_test_me_dir, save_test_file_res_dir)
    time_end = time.time()
