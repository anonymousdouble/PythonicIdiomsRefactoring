    ...
    # for file in os.listdir(save_test_file_res_dir):
        ...
        for file_html in complicate_code:
            ...
            if file_html in dict_complica_me_list:
                ...
                #     continue
                # if file_html!="https://github.com/horovod/horovod/tree/master/test/utils/common.py":#"https://github.com/alteryx/featuretools/tree/master/featuretools/synthesis/encode_features.py":#"https://github.com/pymc-devs/pymc/tree/master/pymc/sampling.py":#"https://github.com/smicallef/spiderfoot/tree/master//sflib.py":#"https://github.com/smicallef/spiderfoot/tree/master//sfwebui.py":#
                #     continue
                ...
                for cl in complicate_code[file_html]:
                    ...
                    for me in complicate_code[file_html][cl]:
                        ...
                        if dict_complica_me_list[file_html]:
                            print(">>>>>>dict_complica_me_list[file_html]: ",dict_complica_me_list[file_html])
                        if total_name in dict_complica_me_list[file_html]:
                            ...
                            if complicate_code[file_html][cl][me] and test_me_inf_list:
                                ...
                                for ind, (old_tree, new_tree) in enumerate(
                                    ...
                                    old_content, new_content,flag_same = replace_content_by_ast.replace_content_chain_compar(repo_name, file_html, old_tree,new_tree)
                                    if flag_same:
                                    ...
                                    try:
                                        for test_html,each_rela_path, cl, me in test_me_inf_list:
                                            # if not ("test_step.py" in test_html and "TestDEMetropolisZ" in cl and "test_tuning_epsilon_parallel" in me):
                                            #     continue
                                            ...
                                            # fun_list = ["TestDEMetropolisZ::test_tuning_epsilon_parallel"]
                                            test_info.append([test_html,cl, me])
                                            # if "test_step.py" not in test_html:
                                            #     continue
                                            ...
                                            if not flag_pass:
                                                ...
                                                print("*******************it is failed: ", ast.unparse(old_tree),ast.unparse(new_tree), run_test_result)
                                                print("****************we save the old content to original file: ", old_path)

                                                res.append(
                                                    [repo_name, file_html, cl, me, old_tree.lineno,
                                            ...
                                            print("****************we save the old content to original file: ", old_path)
                                            res.append(
                                                [repo_name, file_html, cl, me, old_tree.lineno, ast.unparse(old_tree),
...
if __name__ == '__main__':
    ...
    save_me_test_me_dir= util.data_root + "methods_test_method_pair/with_complicated_remove_after_block_use_f/"

    save_test_file_res_dir=util.data_root + "save_test_file_res_dir/chain_comparison/"
    save_test_file_res_dir=util.data_root + "save_test_file_res_dir/with_complicated_remove_after_block_use_f/"

    complicated_code_dir_pkl = util.data_root + "transform_complicate_to_simple_pkl/chain_comparison/"
    complicated_code_dir_pkl= util.data_root + "transform_complicate_to_simple_pkl/with_complicated_remove_after_block_use_f/"

    save_acc_res_csv_dir = util.data_root + "acc_res_compli_test_case/chain_comparison.csv"
    save_acc_res_csv_dir = util.data_root + "acc_res_compli_test_case/with_complicated_remove_after_block_use_f.csv"
    ...
    save_test_acc_dir = util.data_root + "test_case_benchmark_dir/with_complicated_remove_after_block_use_f/"

    pro_path = util.data_root + "python_star_2000repo/"
    # repo_list=[]
    # for file in os.listdir(save_test_file_res_dir):
    #     # if file!="pymc.pkl" :#and file!="featuretools.pkl" :#"featuretools.pkl":
    #     #     continue
    ...
    # repo_list = []
    # for file in os.listdir(save_test_file_res_dir):
    #     if os.path.exists(save_test_acc_dir+file):
    #         continue
    #     repo_list.append(file)
    # "https://github.com/Diaoul/subliminal/tree/master/subliminal/refiners/tvdb.py" 这个是重构前和重构后都是失败的
    print("we need to test: ",len(repo_list))
    # '''
    pool = newPool(nodes=30)
    pool.map(get_each_repo_test_acc, repo_list[:]) # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
