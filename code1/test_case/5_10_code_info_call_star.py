...
def cp_test_file_res_dir_to_for_compre_dict_all(one_test_file, another_test_file, csv_testcase, new_test_file):
    ...
    print("num of new repos after merged: ",
          len(os.listdir(util.data_root + "save_test_file_res_dir/" + new_test_file)))
...
if __name__ == '__main__':
    file_path_list = ["for_compre_list.csv", "for_compre_dict.csv", "for_compre_dict.csv", "for_compre_dict.csv",
                      "for_compre_set.csv", "for_compre_set.csv", "for_compre_set.csv",
                      "call_star.csv", "call_star.csv", "for_target.csv", "for_target.csv", "for_else.csv",
                      "chain_compare.csv", "truth_value_test.csv", "multiple_assign.csv"]
    save_me_test_me_dir_list = ["for_compre_list/", "for_compre_dict/", "for_compre_dict/", "for_compre_dict/",
                                "for_compre_set/", "for_compre_set/", "for_compre_set/",
                                "var_unpack_call_star_complicated/", "var_unpack_call_star_complicated/",
                                "var_unpack_for_target_complicated/", "var_unpack_for_target_complicated/",
                                "for_else_improve_new_modify_bugs_final/",
                                "chain_comparison/", "truth_value_test_complicated_remove_is_is_not/",
                                "multip_assign_complicated/"]
    save_test_file_res_dir_list = ["for_compre_list/", "for_compre_dict/", "for_compre_dict/",
                                   "for_compre_dict_own_config/", "for_compre_set/", "for_compre_set_improve/",
                                   "for_compre_set_all/",
    ...
    complicated_code_dir_pkl_list = ["for_compre_list/", "for_compre_dict/", "for_compre_dict/", "for_compre_dict/",
                                     "for_compre_set/", "for_compre_set/", "for_compre_set/",
                                     "var_unpack_call_star_complicated/", "var_unpack_call_star_complicated/",
                                     "var_unpack_for_target_complicated/", "var_unpack_for_target_complicated/",
                                     "transform_complicate_to_simple_pkl_improve_new_modify_bugs_final/for_else/",
                                     "chain_comparison/", "truth_value_test_complicated_remove_is_is_not_no_len/",
                                     "multip_assign_complicated/"
                                     ]
    save_test_acc_dir_list = ["for_compre_list_acc_dir/", "for_compre_dict_acc_dir/", "for_compre_dict_acc_dir/",
                              "for_compre_dict_acc_dir/", "for_compre_set_acc_dir/", "for_compre_set_acc_dir/",
                              "for_compre_set_acc_dir/",
                              "var_unpack_call_star_complicated_dir/", "var_unpack_call_star_complicated_dir/",
                              "var_unpack_for_target_complicated_dir/", "var_unpack_for_target_complicated_dir/",
    ...
    # '''
    file_save_name=["call_star","call_star_own_config"]
    list_code_info_list = []
    for i in range(len(file_path_list)):
        if i != 8 and i != 7:  # call star
            continue
        if i==8:
            file_save_name="call_star_own_config"
        else:
            file_save_name = "call_star"
        ...
        complicated_code_dir_pkl = util.data_root + "transform_complicate_to_simple_pkl/" + \
                                   complicated_code_dir_pkl_list[i]
                            ...
                            if total_name in dict_complica_me_list[file_html]:
                                ...
                                if complicate_code[file_html][cl][me] and test_me_inf_list:
                                    ...
                                    for ind, (arg_list, new_tree) in enumerate(complicate_code[file_html][cl][me]):
                                        ...
                                        str_info = "".join([file_html, str(arg_list[0][0].lineno)])
                                        # print("str_info: ",str_info)
                                        if str_info in code_info_list:
                                            code_count += 1
                                            if repo_name not in dict_list_compreh_3215:
                                                dict_list_compreh_3215[repo_name] = dict()
                                            if file_html not in dict_list_compreh_3215[repo_name]:
                                                dict_list_compreh_3215[repo_name][file_html] = dict()
                                            if cl not in dict_list_compreh_3215[repo_name][file_html]:
                                                dict_list_compreh_3215[repo_name][file_html][cl] = dict()
                                            if me not in dict_list_compreh_3215[repo_name][file_html][cl]:
                                                dict_list_compreh_3215[repo_name][file_html][cl][me] = []
                                            dict_list_compreh_3215[repo_name][file_html][cl][me].append(
                                                [arg_list, new_tree])
                                            # code_info_cla = code_info.CodeInfo(repo_name, file_html, cl, me,
                                            #                                    [arg_list, new_tree], test_me_inf_list)
                                            # if i == 2 or i == 4:
        ...
        # util.save
        evaluation_testing_dir = util.data_root + "5_10_all_test_cases/code_info_3215/"+file_save_name+"/"  # test_case_benchmark_dir_csv/"#
        for repo_name in dict_list_compreh_3215:
            util.save_pkl(evaluation_testing_dir, repo_name, dict_list_compreh_3215[repo_name])

    print("code count: ", code_count, len(list_code_info_list), total_num)
