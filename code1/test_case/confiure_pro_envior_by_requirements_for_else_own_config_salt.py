...
if __name__ == '__main__':
    ...
    for file_name in os.listdir(save_me_test_me_dir):
        ...
        if repo_name!="salt":#"sympy":
            continue
        repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir,"venv_zejun_salt_config",True])
        # if os.path.exists(save_me_test_me_dir + repo_name + ".pkl"):
        #     repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir])
    print("repo_list: ",repo_list)
    pool = newPool(nodes=30)
    pool.map(configure_pro_envir_util_salt_own_config.save_repo_test_result, repo_list)  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    time_end = time.time()
    print("configure project total time: ", time_end - time_start)

    configure_pro_envir_util_salt_own_config.get_test_success_count(save_me_test_me_dir, save_test_file_res_dir)
    time_end = time.time()
