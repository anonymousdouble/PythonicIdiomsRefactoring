    ...
    for file in os.listdir(save_test_file_res_dir):
        ...
        for test_html in dict_test_html_each_me_res:
            if dict_test_html_each_me_res[test_html]:
                ...
                if flag_test_html:
                    # print("test case is successful: ",test_html,dict_test_html_each_me_res[test_html])
                    flag_repo=1
                else:
                    print("test case is failed: ",test_html,dict_test_html_each_me_res[test_html])
...
if __name__ == '__main__':
    ...
    repo_list = []
    count=0
    repos_test_case = util.load_json(util.data_root, 'test_case_repos')
    for file_name in os.listdir(save_me_test_me_dir):
        repo_name = file_name[:-4]
        if repo_name not in repos_test_case:
            continue
        if os.path.exists(save_me_test_me_dir + repo_name + ".pkl") and not os.path.exists(
            ...
            #     break
            # repo_list.append(
            #         [repo_name, save_me_test_me_dir, save_test_file_res_dir])
            repo_list.append([repo_name, save_me_test_me_dir, save_test_file_res_dir, "venv_test_require_new_", False,
                              pip_install_again])
            count += 1
    ...
    # '''
    get_test_success_count()
    # configure_pro_envir_util.save_repo_test_result()
    # '''
    '''
    for repo_name in dict_repo_file_python:
