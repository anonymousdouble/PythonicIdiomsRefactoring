...
def save_code_map_test_cases(repo_name):
    ...
    for file_html,code_cl,code_me, (old_tree, new_tree) in dict_test_case_inf_selct[repo_name]:
        ...
        old_content, new_content, flag_same = \
            c_replace_content_by_ast_5_10.replace_content_chain_compar_insert_comehere(repo_name, file_html, old_tree,new_tree)
...
if __name__ == '__main__':
    ...
    dict_code_map_pass_test_case_code_dir = util.data_root + "5_10_all_test_cases/select/map_code_info_test_case/chain_compare/"
    ...
    pool.map(save_code_map_test_cases, remain_list[1:])  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
    '''
    #'''
