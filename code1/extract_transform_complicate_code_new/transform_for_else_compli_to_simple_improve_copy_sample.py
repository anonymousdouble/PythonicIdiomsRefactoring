    ...
    if hasattr(node, "body"):
        ...
        for ind, e in enumerate(node.body):
            ...
            # break
                ...
                break_flag = whether_contain_break_and_const_assign(e, assign_list_in_for, if_list_in_for,
        ...
        for ind, child in enumerate(tree.body):
            ...
            if isinstance(child, (ast.For, ast.While)):
                ...
                if not if_varnode or (
                        isinstance(if_varnode, ast.BoolOp) and isinstance(if_varnode.op, (ast.And, ast.Or))):
                    traverse_cur_layer(child, code_list, ass_init_list)
                    continue
                ...
                if break_flag:
                    ...
                    if flag_simplify and assign_flag:
                        init_ass_remove_flag = remove_assign(tree_copy, child_copy, tree, child, if_vars_list,
                                                             intersect_infor_ass_list_in_for, intersect_infor_ass_init,
                                                             ind)

                    # if flag_simplify and assign_flag:
                    #     remove_assign(tree_copy, child_copy, tree, child, if_vars_list, intersect_infor_ass_list_in_for,
                    #                   intersect_infor_ass_init, ind)
...
def save_sample(file_html_list):
    result_compli_for_else_list=[]
    for repo_name,file_html,file_path  in file_html_list:
        try:
        ...
        try:
            ...
            for tree, class_name in ana_py.func_def_list:
                ...
                for code in new_code_list:
                    result_compli_for_else_list.append(
                        [repo_name, file_html,code[3].lineno, ast.unparse(code[5]) + "\n" + ast.unparse(code[3]),
                         ast.unparse(code[4]), code[6]])
        except SyntaxError:
            ...
            traceback.print_exc()
    util.save_csv(util.data_root + "result_csv/for_else_improve.csv",
                  result_compli_for_else_list,
                  ["repo_name", "file_html", "lineno", "old_code", "new_code", "remove_flag"])
    #
...
def save_repo_for_else_complicated(repo_name):
    ...
    for ind, file_info in enumerate(dict_repo_file_python[repo_name]):
        ...
        # if file_html!="https://github.com/sympy/sympy/tree/master/sympy/tensor/array/expressions/conv_array_to_matrix.py":#"https://github.com/sympy/sympy/tree/master/sympy/testing/quality_unicode.py":#"#
        #     continue
...
'''
    ...
    save_complicated_code_dir_pkl = util.data_root + "transform_complicate_to_simple_pkl_improve/for_else/"
    ...
    for file_name in os.listdir(save_complicated_code_dir_pkl):
        ...
        repo_file_count, repo_me_count, repo_code_count, repo_all_file_count, repo_all_me_count = get_code_count(
            complicate_code)
        ...
        for file_html in complicate_code:
            for file_info in dict_repo_file_python[repo_name]:
                if file_html == file_info["file_html"]:
                    file_path = file_info["file_path"]
                    break
                ...
                for me in complicate_code[file_html][cl]:
                    if complicate_code[file_html][cl][me]:
                        ...
                        for code in complicate_code[file_html][cl][me]:
                            file_html_list.append([repo_name,file_html,file_path])
                            # code_list.append(
                            #     [tree, tree_copy, break_list_in_for, child, child_copy, intersect_infor_ass_init[0][1],
                            #      init_ass_remove_flag])
                            # result_compli_for_else_list.append(
                            # [repo_name, file_html, cl,me,code1[3].lineno,ast.unparse(code1[5])+"\n"+ast.unparse(code1[3]), ast.unparse(code1[4]),code1[6]])
                            #
                            # # print("html: ",file_html,cl,me,ast.unparse(code1[0]))
                            # result_compli_for_else_list.append(
                            #     [repo_name, cl, me, ast.unparse(code1[0]), ast.unparse(code1[1]), file_html])
    ...
    random.shuffle(file_html_list)
    save_sample(file_html_list[:400])
    # util.save_csv(util.data_root + "result_csv/for_else.csv",
    #               result_compli_for_else_list[:100],
