...
def transform_left_right(pos1, pos2, node1, node2):
    code = ""
    # when  timeout is not None and 3-2>timeout it cannot transformed 3-2>timeout is not None
    # 这里是处理
    # print("pos1, pos2, node1, node2: ",pos1, pos2, ast.unparse(node1), ast.unparse(node2))
    # 把判断是否为None的node都放在第一个节点node1，并且node1.left=None,因为这种情况下我们不能对node1进行移动或者交换
    node1_still_flag=0
    ...
    for ind, one_var in enumerate(vars_list):
        ...
        for ind_next in range(ind + 1, len(vars_list)):
            ...
            if set(one_var[0]) & set(another_var[0]):
                ...
                print("code1 of node1 and node2: ",ast.unparse(one_var[1]),ast.unparse(another_var[1]),one_pos,another_pos)
        ...
        if isinstance(node, ast.BoolOp):
            ...
            if isinstance(op, ast.And):
                ...
                while 1:
                    ...
                    print(">>new_node: ",ast.unparse(new_node))
                    # if new_node
                    if not flag:
                        break
                    else:
