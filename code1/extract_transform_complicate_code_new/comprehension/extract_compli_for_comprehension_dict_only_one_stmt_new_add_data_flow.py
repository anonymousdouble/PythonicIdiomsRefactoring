...
def tranform_to_one_line(for_body_list):
    ...
    for ind,e in enumerate(for_body_list_copy[:]):
        ...
        if isinstance(e,ast.Assign) and ind<len(for_body_list_copy)-1:
            ...
            for key, replace_value in Map_var.items():
                for key_node in ast.walk(ast.parse(key)):
                    ...
                    return False, []
                # print(">>>tar_node",tar_node,ast.unparse(tar_node),unused_var)
                flag_occur = occur(e, tar_node)
                if flag_occur:
                    rw = Rewrite(tar_node, replace_value)
                    e = rw.visit(e)
        ...
        if isinstance(node, ast.For):
            ...
            if for_traverse(node,assign_block_list):
                ...
                code_index_start_end_list.append(
                        [node, assign_stmt,remove_ass_flag,extend_flag])#, node.lineno, node.end_lineno, assign_stmt_lineno, assign_block_list_str])
            ...
            for tree, class_name in ana_py.func_def_list:
                ...
                for ind, (for_node, assign_node, remove_ass_flag, extend_flag) in enumerate(new_code_list):
                    new_code_list[ind].append(
                        transform_comprehension_dict_compli_to_simple.transform(for_node, assign_node, extend_flag))
