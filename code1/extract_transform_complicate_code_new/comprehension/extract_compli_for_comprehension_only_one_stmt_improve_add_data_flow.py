...
def if_traverse(one_body,assign_block_list,new_for_node_body_list):
    orelse = one_body.orelse
    if_body=one_body.body
    if not orelse:
        return for_traverse(one_body,assign_block_list,new_for_node_body_list)
    else:
...
def tranform_to_one_line(for_body_list):
    ...
    for ind,e in enumerate(for_body_list_copy[:]):
        ...
        # replace the stmt as new stmt
        # pass


def for_traverse(node,assign_block_list,new_for_node_body_list=[]):
    for_body_list = node.body
    # print("come for_traverse: ",ast.unparse(node))#,new_for_node_body_list
    ...
    if len(for_body_list)>1:
        ...
        # try:
            ...
            new_for_node_body_list.extend(copy.deepcopy(for_body_list))
        ...
        #     return False
    if len(for_body_list) == 1:
        # print(">>>>>come len(for_body_list) == 1: ",ast.unparse(node),ast.unparse(for_body_list))
        one_body=for_body_list[0]
        # print("one body: ",ast.unparse(one_body))
        if isinstance(one_body, ast.For):
            # print("come here for_traverse")
            return for_traverse(one_body,assign_block_list,new_for_node_body_list)
        elif isinstance(one_body, ast.If):
            # print("it is if node")
            return if_traverse(one_body,assign_block_list,new_for_node_body_list)
...
def get_complicated_for_comprehen_code_list(tree,content=None,const_empty_list=["[]"],const_func_name="append"):
    ...
    for ind_line,node in enumerate(ast.walk(tree)):
        ...
        try:
            if isinstance(node, ast.For):
                ...
                assign_block_list=[]
                new_for_node_body_list=[]
                # print("it is what: ",ast.unparse(node))
                flag_for=for_traverse(node, assign_block_list, new_for_node_body_list)
                # print("it is what: ", for_traverse(node, assign_block_list,new_for_node_body_list))
                if flag_for:#判断for语句是否满足For_Match
                    ...
                    if flag!=2 and flag!=3:
                        ...
                        continue
                    # print("flag: ", assign_stmt,assign_stmt_lineno)
                    extend_flag=0 if flag!=3 else 1
                    ...
                    # print(assign_stmt+"\n"+ast.unparse(node))
                    code_index_start_end_list.append([node,assign_stmt,remove_ass_flag,new_for_node_body_list,extend_flag])
...
'''
    ...
    for file_name in os.listdir(save_complicated_code_dir_pkl):
        ...
        repo_exist =1 if repo_code_count else 0
        '''
        repo_exist = 0
