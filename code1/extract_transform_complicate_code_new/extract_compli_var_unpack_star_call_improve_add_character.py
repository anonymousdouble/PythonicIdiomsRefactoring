...
def get_func_call_by_args_character(tree):
    code_list = []
    arg_same_list = []
    new_arg_same_list = []
        ...
        if isinstance(node, ast.Call):
            ...
            for ind_elts, e in enumerate(args_list):
                ...
                if isinstance(e, ast.Constant) and isinstance(e.value,str) and len(e.value)==1:
                    ...
                    if len(arg_one_list)>1:
                        new_arg_same_list.append([arg_one_list, [], node, 0])
                        # try:
                        #     new_node_star=new_node(arg_one_list)
                        #     if new_node_star:
                        #         new_arg_same_list.append([arg_one_list, [], node, 0])
                        #     arg_one_list = []
                        #
