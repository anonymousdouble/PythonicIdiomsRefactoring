...
def is_same_two_if_test(test_1, test_2):
    ...
    if isinstance(test_1, ast.Compare) and isinstance(test_2, ast.Compare):
        ...
        empty_list = ["None", "False", "0", "0.0", "0j", "Decimal(0)", "Fraction(0, 1)", '', "()", "[]", "{}",
                      "dict()", "set()", "range(0)"]

        # print("not compare: ",ast.unparse(test_1),ast.unparse(test_2))

        if isinstance(test_1, ast.Compare):
            # print(">>>test_2: ",test_2)
            comparators = test_1.comparators + [test_1.left]
            if isinstance(test_2, ast.UnaryOp):
                same_comparator = ast.unparse(test_2.operand)
            else:
                same_comparator = ast.unparse(test_2)
            if same_comparator in ast.unparse(comparators):
                for comp in comparators:
                    if same_comparator != ast.unparse(comp) not in empty_list:
                        test_1_str = same_comparator
                        test_2_str = ast.unparse(test_2)

                        break

        elif isinstance(test_2, ast.Compare):
            # print(">>>test_1: ",test_1)
            comparators = test_2.comparators + [test_2.left]
            if isinstance(test_1, ast.UnaryOp):
                same_comparator = ast.unparse(test_1.operand)
            else:
                same_comparator = ast.unparse(test_1)
            if same_comparator in ast.unparse(comparators):
                for comp in comparators:
                    if same_comparator != ast.unparse(comp) not in empty_list:
                        test_2_str = same_comparator
                        test_1_str = ast.unparse(test_1)
                        break
        else:
            test_1_str = ast.unparse(test_1)
            test_2_str = ast.unparse(test_2)
        if test_1_str and test_2_str:
            # print("test_1_str, test_2_str: ",test_1_str, test_2_str)
...
def is_differ_two_if_test(test_1, test_2):
    ...
    if isinstance(test_1, ast.Compare) and isinstance(test_2, ast.Compare):
        ...
        empty_list = ["None", "False", "0", "0.0", "0j", "Decimal(0)", "Fraction(0, 1)", '', "()", "[]", "{}",
                      "dict()", "set()", "range(0)"]
        empty_list = ["None", "True", "False", "0", "0.0", "0j", "Decimal(0)", "Fraction(0, 1)", '', "()", "[]", "{}",
                      "dict()", "set()", "range(0)"]
        ...
        if test_1_str and test_2_str:
            # print(">>>test_1_str,test_2_str: ", test_2_str, test_1_str)
...
def traverse_func(tree,code_list,ass_init_list):
    for ind, child in enumerate(tree.body):
        ...
        if isinstance(child, (ast.For, ast.While)):
            ...
            # print(">>>come here: ", flag_if_var, if_varnode,ast.unparse(if_varnode))

            # ignore this code1 if a and b or c:
            ...
            if_list_in_for = []
            break_list_in_for = []
            break_flag = whether_contain_break_and_const_assign(child_copy, all_assign_list_in_for, if_list_in_for,
                                                                break_list_in_for)
            flag_simplify = 1
            assign_flag = 1
            if break_flag:
                # print(">>>>come break: ",len(if_list_in_for),ast.unparse(if_list_in_for))
                ...
                if not intersect_flag_ass_init or len(intersect_infor_ass_init[-1][-1]) > 1:
                    # print("not intersect_flag_ass_init: ",intersect_infor_ass_init,ast.unparse(child))
                    flag_simplify = 0
                    if not intersect_flag_ass_init:
                        if_list_each=if_list_in_for[0]
                        if hasattr(if_list_each, "test"):
                            ...
                            if is_differ_two_if_test(if_varnode.test, if_list_each.test):
                                ...
                                code_list.append(
                                    [tree, tree_copy, break_list_in_for, child, child_copy,
                                     [],
                                     if_varnode, 0])
                ...
                for ind_test, assign_list_in_for in enumerate(all_assign_list_in_for):
                    ...
                    if flag_inn:
                        ...
                        for line_no, ass, inter_var in info_inn:
                            # print(">>>come here: ", ass, if_varnode, ast.unparse(if_varnode))
                ...
                if flag_simplify and assign_flag:
                    init_ass_remove_flag = remove_assign(tree_copy, child_copy, tree, child, if_vars_list,
                                                         intersect_infor_ass_list_in_for, intersect_infor_ass_init, ind)
                ...
                if flag_simplify:
                    ...
                    code_list.append(
                        [tree, tree_copy, break_list_in_for, child, child_copy, intersect_infor_ass_init[0][1],
                         if_varnode, init_ass_remove_flag])
...
def traverse_func_orelse(tree,code_list,ass_init_list):
    for ind, child in enumerate(tree.orelse):
        tree_copy = copy.deepcopy(tree)
        child_copy = tree_copy.orelse[ind]
        if isinstance(child, (ast.For, ast.While)):
            if child.orelse:
                traverse_cur_layer(child, code_list, ass_init_list)
                continue
            if ind + 1 >= len(tree.orelse):
                traverse_cur_layer(child, code_list, ass_init_list)
                continue
            if_vars_list, intersect_infor, assign_list_in_for = [], [], []
            # print("tree_copy: ",tree_copy,tree.body,ast.unparse(child),child)
            next_child = tree_copy.orelse[ind + 1]

            flag_if_var, if_varnode = whether_if_and_contain_var(next_child)
            # print(">>>come here: ", flag_if_var, ast.unparse(if_varnode))
            ...
            if break_flag:
                ...
                if not intersect_flag_ass_init or len(intersect_infor_ass_init[-1][-1]) > 1:
                    ...
                    # ass_init的语句应该和for节点同父母或者for节点属于ass_init.parent
                    for ind_chi, ass_child in enumerate(tree.orelse[:ind]):
                        if isinstance(ass_child, (ast.Assign, ast.AnnAssign)):
                            continue
                        else:
                            for ass_child_child in ast.walk(ass_child):
                    ...
                    #     flag_simplify = 0
                    #     break
                    print(">>>come here: ", intersect_infor_ass_init[-1][-1], ast.unparse(if_varnode))
                    ...
                    if is_same_ass_init_if_test(intersect_infor_ass_init[-1][1], if_varnode.test):
                        ...
                        assign_flag = 1
                                                                           ...
                                                                           if_varnode.test):
                        print("yes  is_differ_ass_again_if_test if_test and assign init could be simplified to for else: ",ast.unparse(tree_copy))
                ...
                for ind_test, assign_list_in_for in enumerate(all_assign_list_in_for):
                    ...
                    if flag_inn:
                        ...
                        intersect_infor_ass_list_in_for.append(info_inn)
                        for line_no, ass, inter_var in info_inn:
                            print(">>>come here: ", ast.unparse(ass), if_varnode, ast.unparse(if_varnode))
                            # if ass_init_list and ast.unparse(ass_init_list[-1].value) != ast.unparse(ass.value):
                            if is_differ_ass_again_if_test(ass, if_varnode.test):
                                print(">>>is_differ_ass_again_if_test: ")
                                body_break_flag = modify(tree_copy, child_copy, if_varnode, if_list_each, 1)
                                if body_break_flag:
                                    flag_simplify = 0
                                break
                            elif if_varnode.orelse and is_same_ass_init_if_test(ass, if_varnode.test):
                                print(">>>is_same_ass_init: ")
...
def traverse_cur_layer(tree, code_list, ass_init_list):
    # print(">>>>>>>>>assign_list: ", ast.unparse(ass_init_list))
    if hasattr(tree, "body"):
        traverse_func(tree, code_list, ass_init_list)


    if hasattr(tree, "orelse"):
        # print(">>>>>>>>>orelse assign_list: ", ast.unparse(ass_init_list))

        traverse_func_orelse(tree, code_list, ass_init_list)
