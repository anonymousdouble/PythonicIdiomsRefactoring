            ...
            for ind,e_node in enumerate(slice_list[1:]):
                ...
                if 1:#pre_var_str.isdigit() and e_str.isdigit():
                    ...
                    if not step:
                        ...
                        if not step.isdigit() or (step.isdigit() and int(step)==0) or (e_str.isdigit() and int(e_str)<0):
...
'''
    ...
    save_complicated_code_dir_pkl= util.data_root + "transform_complicate_to_simple_pkl/var_unpack_call_star_complicated_fix_negtive/"
