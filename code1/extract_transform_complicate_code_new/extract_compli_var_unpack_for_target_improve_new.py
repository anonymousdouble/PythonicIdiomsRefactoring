...
from transform_c_s import transform_var_unpack_for_target_compli_to_simple_new
...
def get_for_target(tree):
                ...
                for e_body in ast.walk(node):
                        ...
                        for left_e in left:
                            for ass_e in ast.walk(left_e):
                                ...
                                if ast.unparse(ass_e) == var:
                                    # print("it cannot be simplified because it contains write operation for the var ",var,ast.unparse(node))
                                    break
                            else:
                                continue
                            break
                    ...
                    if flag:
                        for stmt in node.body:
                            ...
                            new_code= transform_var_unpack_for_target_compli_to_simple_new.transform_for_node_var_unpack(node_copy, var_list_real, Map_var)
    ...
    return code_list
def get_for_target_add_info(tree):
    code_list=[]
                ...
                for e_body in ast.walk(node):
                    ...
                    if flag:
                        for stmt in node.body:
                            ...
                            if new_code:
                                # print("new_code: ",ast.unparse(new_code))
                                code_list.append([node,new_code,var_list_real,Map_var])
                            else:
                                code_list.append([node,var_list_real,Map_var])
                                # print(">>>>>>>>>>>the code1 I cannot transform: ",ast.unparse(node))
                        # print("it maybe simplified")
    return code_list
