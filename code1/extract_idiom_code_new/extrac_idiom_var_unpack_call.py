...
def get_idiom_all_starred_call_improve_parent(tree):
    code_info = dict()
    for node in ast.walk(tree):
        if isinstance(node, (ast.stmt)):
            print("come node: ",node)
            for child_node in ast.walk(node):
                # stmt node
                print("come child_node: ", child_node)
                if isinstance(child_node, (ast.Starred)):
                        print("come starred stmt: ", child_node)
                        if child_node in code_info:
                            code_info[child_node][0] = node
                        else:
                            code_info[child_node] = [node, None]
                    # break
                # direct parent node
                for child in ast.iter_child_nodes(child_node):
                    if isinstance(child, ast.Starred):
                            print("come starred direct parent: ", child_node)
                            if child in code_info:
                                code_info[child][1] = child_node
                            else:
                                code_info[child] = [None, child_node]

                            # break
    if code_info:
        print("code_info: ", code_info)
    return code_info
def get_idiom_all_starred_call_improve(tree):

    code_list = []
    repeat_call_list=[]
    for w_node in ast.walk(tree):
        if isinstance(w_node,ast.stmt):
            for node in ast.walk(w_node):
                if isinstance(node, ast.Call):
                    args_list = node.args
                    for ind_elts, e in enumerate(args_list):
                        if isinstance(e, ast.Starred):
                            if e not in repeat_call_list:
                                code_list.append([node,e,w_node])
                                repeat_call_list.append(e)
                            else:
                                index=repeat_call_list.index(e)
                                code_list[index][-1]=w_node

    return code_list
def get_idiom_starred_call_improve(tree):
    code_list = []
    repeat_call_list=[]
    for w_node in ast.walk(tree):
        if isinstance(w_node,ast.stmt):
            ...
            for ind_elts, e in enumerate(args_list):
                if isinstance(e, ast.Starred):
                    ...
                    if ast.unparse(value)!="args":
                        code_list.append([ast.unparse(e),ast.unparse(node)])
    return code_list
def get_idiom_starred_no_args_call_subscript_add_call(tree):
    code_list = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            args_list = node.args
            for ind_elts, e in enumerate(args_list):
                if isinstance(e, ast.Starred):
                    value=e.value
                    if ast.unparse(value)!="args":
                        code_list.append([ast.unparse(node),ast.unparse(e),ast.unparse(node)])
    return code_list
...
'''
    ...
    util.save_csv(util.data_root+"idiom_code_dir_star_1000/var_unpack_call_idiom_code_subscript_slice.csv",result_compli_for_else_list,["repo_name","code1","code_call","file_html","file_path"])

    import random

    #random.shuffle(result_compli_for_else_list)
    #util.save_csv(util.data_root+"idiom_code_dir_star_1000/var_unpack_call_idiom_code.csv",result_compli_for_else_list,["repo_name","code1","code_call","file_html","file_path"])
   #
    #util.save_csv(util.data_root+"idiom_code_dir_star_1000/var_unpack_call_idiom_code_sample.csv",result_compli_for_else_list[:380],["repo_name","code1","code_call","file_html","file_path"])
    '''
    files_num_list = []
    star_num_list = []
