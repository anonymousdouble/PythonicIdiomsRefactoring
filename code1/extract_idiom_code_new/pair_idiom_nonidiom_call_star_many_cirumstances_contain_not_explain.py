...
from extrac_idiom_var_unpack_call import  get_idiom_starred_call_improve
from transform_call_star_s_c import transform_idiom_call_star
...
def save_to_csv(save_complicated_code_dir_pkl):
                ...
                for me in complicate_code[file_html][cl]:
                    if complicate_code[file_html][cl][me]:
                        for code in complicate_code[file_html][cl][me]:
                            ...
                            #     [repo_name, file_html, cl, me, ast.unparse(code[0]),ast.unparse(code[1]),code[2],code[3]])
                            result_compli_for_else_list.append(
    ...
    util.save_pkl(util.data_root + "idiom_code_dir_star_1000_csv/", "star_call_idiom_code_many_circumstances_cannot_explain",
                  result_compli_for_else_list)
    util.save_csv(util.data_root+"idiom_code_dir_star_1000_csv/star_call_idiom_code_many_circumstances_cannot_explain.csv",result_compli_for_else_list,["repo_name","file_html","file_path","complicate_code","call_func","simple_code","if_refactorable"])
...
def parse_a_tree(tree):
    code_list = []
    old_code_list = get_idiom_starred_call_improve(tree)
    for code in old_code_list:
        code_list.append(transform_idiom_call_star(code[0],code[1]))
    print(code_list)
    return code_list
    ...
    for file_info in dict_repo_file_python[repo_name]:
        ...
        try:
            ...
            for tree, class_name in ana_py.func_def_list:
                ...
                old_code_list=get_idiom_starred_call_improve(tree)
                for code in old_code_list:
                    each_code_pair=transform_idiom_call_star(code[0],code[1],code[2])
                    if each_code_pair:
...
if __name__ == '__main__':
    ...
    code='''
tf.group(*[var_update, m_t, v_t])
np.random.rand(*(10, 1))
zip(*labels_and_scores[:top_k])
nn.Sequential(*self.features)
zip(*[column_major[c] for c in col_names])
keys.difference_update(*map(set, args[1:]))#https://github.com/Erotemic/ubelt/tree/master/ubelt/util_dict.py
set.intersection(*map(set, args)) #https://github.com/Erotemic/ubelt/tree/master/ubelt/util_dict.py
it.product(*basis.values()) #https://github.com/Erotemic/ubelt/tree/master/ubelt/util_dict.py
cv2.VideoWriter_fourcc(*'avc1')#https://github.com/micah5/ace-attorney-reddit-bot/tree/master//anim.py
(a,b)=1,2  
a,b=b,a  
a,b,c=c,a,b
a,c,*b=[1,2,3]
...
'''
    ...
    # b="j+2"
    step="2"
    step=sympy.sympify(step)
    print("step",type(step),isinstance(step,sympy.core.numbers.Integer))

    #
    # res=sympy.sympify(b+"-("+a+")")
    # print(res,type(res),isinstance(res,sympy.core.numbers.Integer))
    # a = "j"
    # b = "j+i"
    # res = sympy.sympify(b + "-(" + a + ")")
    # print(res,type(res),isinstance(res,sympy.core.numbers.Integer))
    ...
    save_complicated_code_dir_pkl= util.data_root + "idiom_code_dir_pkl_new/call_star_idiom_code_cannot_exaplain/"
