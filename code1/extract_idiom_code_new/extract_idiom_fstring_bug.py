...
def find_for_else(for_code):
    ...
    if e:
        return 1
    return 0
def get_idiom_for_else_improve(tree):
    code_list=[]
    for node in ast.walk(tree):
        if isinstance(node, (ast.For,ast.While)):
            #  查找包含else的节点，并且在其中的非for，while 节点中查找包含break的语句
...
'''
    ...
    pool.map(save_repo_for_else_complicated, repo_list[:10])  # [:3]sample_repo_url ,token_num_list[:1]
    pool.close()
    pool.join()
