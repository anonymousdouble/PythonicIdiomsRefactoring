...
def get_idiom_dict_comprehension_improve(tree):
    code_list = []
    for node in ast.walk(tree):
        if isinstance(node,ast.Assign) and isinstance(node.value,ast.DictComp):
            ...
            for child_node in ast.walk(node):
                if isinstance(child_node,ast.DictComp):
                    code_list.append([node,child_node])
                    break
    return code_list

def get_idiom_dict_comprehension(tree):
    code_list = []
    for node in ast.walk(tree):
    ...
    for file_info in dict_repo_file_python[repo_name]:
        ...
        try:
            ...
            for tree, class_name in ana_py.func_def_list:
                code_list = get_idiom_dict_comprehension(tree)
                if code_list:
            ...
            #
            #     code_list.extend(get_idiom_dict_comprehension(tree))
            # if code_list:
...
'''
    ...
    # save_complicated_code_dir = util.data_root + "idiom_code_dir_star_1000/dict_comprehension_idiom_code/"
    save_complicated_code_dir_pkl= util.data_root + "idiom_code_dir_pkl/dict_comprehension_idiom_code/"
    ...
    # util.save_csv(util.data_root+"idiom_code_dir_star_1000/dict_comprehension_idiom_code.csv",result_compli_for_else_list,["repo_name","code1","line_no","file_html","file_path"])
