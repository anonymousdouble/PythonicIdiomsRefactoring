...
def get_idiom_chained_comparison_improve_add_parent_node(tree):
    ...
    return code_info
def get_idiom_chained_comparison_improve(tree):
    code_list = []
    for node in ast.walk(tree):
    ...
    return code_list
def get_idiom_chained_comparison(tree):
    code_list = []
    for node in ast.walk(tree):
        if isinstance(node, (ast.Compare)):
            ops=node.ops
            if len(ops)>1:
                ...
                #left_code_node.lineno,left_code_node.end_lineno,left_code_node.col_offset,left_code_node.end_col_offset
                code_list.append([ast.unparse(node),[node.lineno,node.end_lineno,node.col_offset,node.end_col_offset]])
...
'''
    ...
    save_complicated_code_dir= util.data_root + "idiom_code_dir_star_1000/chained_comparison_idiom_code/"
    save_complicated_code_dir_pkl= util.data_root + "idiom_code_dir_pkl/chained_comparison_idiom_code/"
    repo_list = []
    ...
    # util.save_csv(util.data_root+"idiom_code_dir_star_1000/chained_comparison_idiom_code.csv",result_compli_for_else_list,["repo_name","code1","line_no","file_html","file_path"])
