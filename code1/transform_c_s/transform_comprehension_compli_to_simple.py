...
def test_fun(a):
    a.id="change"
    print()
def transform(for_node, assign_node,extend_flag=0):
    if not extend_flag:
        new_code=copy.deepcopy(assign_node)

        listcompre=ast.ListComp()
        ...
        listcompre.elt = None
        for_transform(for_node, listcompre)
        new_code_str=ast.unparse(new_code.targets[0])+".extend("+ast.unparse(listcompre)+")"
        for e in ast.walk(ast.parse(new_code_str)):
