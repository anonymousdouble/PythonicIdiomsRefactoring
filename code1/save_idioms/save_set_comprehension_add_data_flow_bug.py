...
import util
import shutil
from wrap_refactoring import refactor_set_comprehension_add_data_flow
from extract_transform_complicate_code_new.comprehension import extract_compli_for_comprehension_logic
...
for i in range(4262441):
    if i!=162321:#17302 72164 75793
        continue
    file_name=f"_{i}.py"
    file_path=method_dir+file_name
    new_code_list=refactor_set_comprehension_add_data_flow.refactor_set_comprehension(file_path)
    ...
    for node in ast.walk(ast.parse(content)):# 可能的non-idiomatic code
        if isinstance(node,(ast.For,ast.AsyncFor)):
            for e in node.body:
                if extract_compli_for_comprehension_logic.whether_fun_is_append(e,[],const_func_name="add"):
