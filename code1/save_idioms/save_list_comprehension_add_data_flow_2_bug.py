...
from extract_transform_complicate_code_new.comprehension import extract_compli_for_comprehension_only_one_stmt_improve_add_data_flow
...
list_comprehension_method_dir=util.data_root+"make_benchmark_idiomatic_list_comprehension_new_3/"
...
for i in range(4262441):
    if i !=502: #3277 3673 1054 1054 1750 1825 2772
        continue
    file_name=f"_{i}.py"
    ...
    for node in ast.walk(ast.parse(content)):# 可能的non-idiomatic code
        if isinstance(node,(ast.For,ast.AsyncFor)):
            for e in node.body:
                if extract_compli_for_comprehension_only_one_stmt_improve_add_data_flow.whether_fun_is_append(e, []):
