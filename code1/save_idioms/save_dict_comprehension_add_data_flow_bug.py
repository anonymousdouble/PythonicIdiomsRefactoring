...
import util
import shutil
from wrap_refactoring import refactor_dict_comprehension_add_data_flow
from extract_transform_complicate_code_new.comprehension import extract_compli_for_comprehension_dict_only_one_stmt_new_add_data_flow
...
list_comprehension_method_dir=util.data_root+f"make_benchmark_idiomatic_{idiom}_add_data_flow/"
...
for i in range(4262441):
    if i !=335:#6448:#941 1496 1826 3070 4127 5723 5776
        continue
    file_name=f"_{i}.py"
    file_path=method_dir+file_name
    new_code_list=refactor_dict_comprehension_add_data_flow.refactor_dict_comprehension(file_path)
    ...
    for node in ast.walk(ast.parse(content)):# 可能的non-idiomatic code
        if isinstance(node,(ast.For,ast.AsyncFor)):
            for e in node.body:
                if extract_compli_for_comprehension_dict_only_one_stmt_new_add_data_flow.whether_fun_is_append(e,[]):
