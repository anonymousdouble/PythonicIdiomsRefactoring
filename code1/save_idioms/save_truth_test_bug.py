...
import util
import shutil
from wrap_refactoring import refactor_truth_value_test
...
for i in range(4262441):
    ...
    new_code_list=refactor_truth_value_test.refactor_truth_value_test(file_path)
    if new_code_list:
        for e in new_code_list:
            print(">>>>new_code_list: ",ast.unparse(e[0]),ast.unparse(e[1]))
    break
    # else:
    #     continue
    ...
    for node in ast.walk(tree):# 可能的non-idiomatic code
        ...
        if isinstance(node, ast.Compare):
            if extract_compli_truth_value_test_code_remove_is_isnot_remove_len.decide_compare_complicate_truth_value(node):
