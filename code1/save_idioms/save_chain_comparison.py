...
import util
import shutil
from wrap_refactoring import refactor_chain_compare
...
for i in range(4262441):
    ...
    new_code_list=refactor_chain_compare.refactor_chain_compare(file_path)
    # if new_code_list:
    ...
    for node in ast.walk(tree):# 可能的non-idiomatic code
        ...
        if isinstance(node, ast.Compare):
            ...
            vars = [[left_str, right_str, node]]

            for e_left_str,e_right_str,e_com_node in vars_list:

                if e_left_str==left_str or right_str==e_right_str:
                    if ast.unparse(e_com_node) in ridiom_comp_list and ast.unparse(node) in ridiom_comp_list :
                        # print(">>>>>come here")
                        continue
                    # print(">>>>>come here")
                    parent_node_1 = find_parent_node(tree, e_com_node)
                    parent_node_2 = find_parent_node(tree, node)
                    # print(">>>parent_node_1: ",ast.unparse(parent_node_1))
                    # print(">>>parent_node_2: ",ast.unparse(parent_node_2))

                    if hasattr(parent_node_1,"lineno") and hasattr(parent_node_2,"lineno"):
                        if parent_node_1.lineno>=parent_node_2.lineno>=parent_node_1.end_lineno:

                                need_to_check_file_name_excel.append(
                                    [file_name, node.lineno, e_com_node.lineno, ast.unparse(parent_node_2),
                                     ast.unparse(node), ast.unparse(parent_node_1), ast.unparse(e_com_node)])
                                method_info.append(str(node.lineno))
                        elif parent_node_2.lineno>=parent_node_1.lineno>=parent_node_2.end_lineno:
                                need_to_check_file_name_excel.append(
                                    [file_name, node.lineno, e_com_node.lineno, ast.unparse(parent_node_2),
                                     ast.unparse(node), ast.unparse(parent_node_1), ast.unparse(e_com_node)])
                                method_info.append(str(node.lineno))
                    else:
                        for e_parent_node in ast.walk(parent_node_1):
                            if e_parent_node ==parent_node_2:
                                need_to_check_file_name_excel.append(
                                    [file_name, node.lineno, e_com_node.lineno, ast.unparse(parent_node_2),
                                     ast.unparse(node), ast.unparse(parent_node_1), ast.unparse(e_com_node)])
                                method_info.append(str(node.lineno))
                            ...
                            for e_parent_node in ast.walk(parent_node_2):
                                if e_parent_node == parent_node_1:
                                    need_to_check_file_name_excel.append(
                                        [file_name, node.lineno, e_com_node.lineno, ast.unparse(parent_node_2),
                                         ast.unparse(node), ast.unparse(parent_node_1), ast.unparse(e_com_node)])
                                    method_info.append(str(node.lineno))
            ...
            #
            # need_to_check_file_name_excel.extend([[file_name,ast.unparse(node),ast.unparse(e_com_node),node.lineno,e_com_node.lineno] for e_left_str,e_right_str,e_com_node in vars_list if e_left_str==left_str or right_str==e_right_str])
