...
import util
import shutil
from wrap_refactoring import refactor_fstring
...
for i in range(4262441):
    ...
    new_code_list=refactor_fstring.refactor_fstring(file_path)
    # if new_code_list:
    ...
    lineno_list=[e.lineno for e,_ in new_code_list]
    # ridiom_comp_list=[ast.unparse(e[0]) for e in new_code_list]
    compare_node=[]
    # print(">>>>>>new_code_list:",new_code_list)
    all_info=[[file_name,e.lineno,ast.unparse(e),ast.unparse(new_ass)] for e,new_ass in new_code_list]
    if new_code_list:
