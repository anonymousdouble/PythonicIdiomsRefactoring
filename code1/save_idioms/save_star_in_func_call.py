...
import util
import shutil
from wrap_refactoring import refactor_call_star
...
for i in range(4262441):
    ...
    new_code_list=refactor_call_star.refactor_call_star(file_path)
    # if new_code_list:
    #     print(">>>>new_code_list: ",new_code_list[0][0][-2])#", ".join([arg for arg in new_code_list[0][0]]))#,ast.unparse(new_code_list[0][1]))
    #     # break
    # else:
    #     continue
    ...
    compare_node=[]
    all_info=[[file_name,e[0][-2].lineno,", ".join([ast.unparse(arg) for arg in e[0][0]]),ast.unparse(e[-1])] for e in new_code_list]
